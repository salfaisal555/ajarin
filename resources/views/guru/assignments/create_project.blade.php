<x-app-layout>
        ...
        <form action="{{ route('assignments.store_project', $course->id) }}" method="POST">
            ...
            <div class="bg-white p-6 rounded-xl shadow-md mb-6 border border-gray-100">
                <div class="flex justify-between items-center mb-6 border-b pb-4">
                    ...
                    <button type="button" onclick="addGroup()" class="px-4 py-2 bg-indigo-100 text-indigo-700 font-bold rounded-lg hover:bg-indigo-200 transition">
                        + Tambah Kelompok
                    </button>
                </div>
                        ...
                        <div class="max-h-40 overflow-y-auto space-y-2 bg-white p-3 rounded-lg border border-gray-200">
                            @forelse($students as $student)
                                <label class="flex items-center space-x-3 cursor-pointer p-1 hover:bg-gray-50 rounded">
                                    <input type="checkbox" name="groups[0][students][]" value="{{ $student->id }}" class="rounded text-indigo-600 focus:ring-indigo-500">
            ...
            <div class="mt-6 text-right">
                <button type="submit" class="px-8 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 shadow-lg">
    ...
    <script>
        ...
        function addGroup() {
            ...
            // Kita ambil HTML list siswa dari Blade yang sudah di-render, lalu kita generate checkbox-nya pakai JS
            const studentsHtml = `
